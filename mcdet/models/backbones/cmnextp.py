    ...
    def forward(self, x: list) -> list:
        ...
        for i in range(4):
            ...
            if self.num_modals > 0:
                ...
                x_cam_rect, x_f_rect = self.FRMs[i](x_cam_rect, x_f_processed)
                x_fused = self.FFMs[i](x_cam_rect, x_f_rect)
                
                # Hook을 통한 시각화/통계 로깅
                if hasattr(self, '_visualization_hook') and self._visualization_hook is not None:
                    if x_scores is not None and attention_weights is not None:
                        # Soft Mix 통계 로깅 (CMNeXtP)
                        self._visualization_hook.log_soft_mix_stats(
                            attention_map=attention_weights,
                            modality_names=self.modals,
                            log_prefix=f"stage{i+1}",
                            batch_idx=0
                        )
                    
                    # Feature map 시각화
                    feature_maps = {
                        'PPX_features': x_f_processed,
                        'FRM_input': x_cam_out,
                        'FFM_output': x_fused
                    }
                    self._visualization_hook.log_feature_maps(
        ...
        # Hook을 통한 시각화/통계 로깅 (모델 클래스에서는 제거)
        # 시각화 관련 코드는 CMNeXtVisualizationHook에서 처리

        return outs
    
# 시각화 관련 함수들은 CMNeXtVisualizationHook으로 이동됨