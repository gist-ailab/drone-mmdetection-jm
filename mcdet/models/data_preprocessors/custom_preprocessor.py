...
class MultiModalDetDataPreprocessor(ImgDataPreprocessor):
    ...
    def pre_forward(self, data: dict, training: bool = False, key: str = 'inputs_rgb') -> dict:
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        data = self.pre_forward(data=data, training=training, key='inputs_rgb')
        data = self.pre_forward(data=data, training=training, key='inputs_ir')
        inputs_rgb, inputs_ir, data_samples = data['inputs_rgb'], data['inputs_ir'], data['data_samples']
        ...
        if training and self.batch_augments is not None:
            for batch_aug in self.batch_augments:
                inputs_rgb, inputs_ir, data_samples = batch_aug(inputs_rgb, inputs_ir, data_samples)

        return {'inputs_rgb': inputs_rgb, 'inputs_ir': inputs_ir, 'data_samples': data_samples}

    def _get_pad_shape(self, data: dict) -> List[tuple]:
        """Get the pad_shape of each image based on data and
        pad_size_divisor."""
        _batch_inputs = data['inputs_rgb']
        # Process data with `pseudo_collate`.
        if is_seq_of(_batch_inputs, torch.Tensor):
            batch_pad_shape = []
