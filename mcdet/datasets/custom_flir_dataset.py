...
class FLIRCatDataset(CocoDataset):
    ...
    def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
        ...
        data_info['thermal_img_path'] = osp.join(self.data_prefix['img'].replace('video_rgb_test', 'video_thermal_test'), img_info['file_name'].replace(img_name, self.rgb2thermal[img_name]))
...
@DATASETS.register_module()
class FLIRCatDataset2(CocoDataset):

    def __init__(self, *args, test_ratio=0.1, **kwargs):
        super(FLIRCatDataset2, self).__init__(*args, **kwargs)
    ...
    def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
        ...
        # TODO: need to change data_prefix['img'] to data_prefix['img_path']
        # img_path = osp.join(self.data_prefix['img'], img_info['file_name'])
        img_path = osp.join(self.data_prefix['img'], img_info['file_name_IR'])
        if self.data_prefix.get('seg', None):
            seg_map_path = osp.join(
                self.data_prefix['seg'],
                img_info['file_name'].rsplit('.', 1)[0] + self.seg_map_suffix)
            ...
            seg_map_path = None
        data_info['img_path'] = img_path
        img_name = os.path.basename(img_info['file_name_IR'])
        # data_info['thermal_img_path'] = osp.join(self.data_prefix['img'].replace('video_rgb_test', 'video_thermal_test'), img_info['file_name'].replace(img_name, self.rgb2thermal[img_name]))
