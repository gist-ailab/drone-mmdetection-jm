...
custom_imports = dict(imports=['mcdet.models.backbones'], allow_failed_imports=False)
...
train_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=f'{data_root}labels/train_heuristic_cropped.json',
        data_prefix=dict(img='heuristic_cropped'),
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=f'{data_root}labels/test_heuristic_cropped.json',
        data_prefix=dict(img='heuristic_cropped'),
...
val_evaluator = dict(
    type='CocoMetric',
    ann_file=os.path.join(data_root, 'labels/test_heuristic_cropped.json'),
    metric='bbox')
...
experiment_name = 'sejong2504_heuristicalign_cropped_cmnextp_b2_rcnn_multiscale_v2'
