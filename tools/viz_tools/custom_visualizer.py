...
def vis_pred(img, result, classes):
    ...
    for i in range(len(bboxes)):
        ...
        score = scores[i]
        label = labels[i]
        color = COLOR_PALETE[label]
        cv2.rectangle(img_result,  (int(bbox[0]), int(bbox[1])), (int(bbox[2]),int(bbox[3])), color, 2)
        cv2.putText(img_result, f'{classes[label]}: {score:.2f}', (int(bbox[0])-2, int(bbox[1])-2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
    return img_result
